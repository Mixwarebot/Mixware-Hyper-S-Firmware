...
G29_TYPE GcodeSuite::G29() {
  ...
  if (homing_needed_error()) {
    IF_ENABLED(TFT_MIXWARE_UI, set_auto_leveling_state(LEVEL_STATE_LEVELERR));
    G29_RETURN(false, false);
  }
  ...
  TERN_(HAS_BED_PROBE, probe.move_z_after_probing());
    ...
    planner.synchronize();
    #if ENABLED(TFT_MIXWARE_UI)
      if (get_auto_leveling_state() == LEVEL_STATE_LEVELERR) {
        process_subcommands_now(F("G28\nM502\nM500"));
      }
      else {
        process_subcommands_now(F(Z_PROBE_END_SCRIPT));
        set_auto_leveling_state(LEVEL_STATE_FINISHED);
      }
    #else
      process_subcommands_now(PSTR(Z_PROBE_END_SCRIPT));
    #endif
    // process_subcommands_now(F(Z_PROBE_END_SCRIPT));
  #endif
