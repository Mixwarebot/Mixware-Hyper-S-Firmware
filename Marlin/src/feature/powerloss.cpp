...
void PrintJobRecovery::save(const bool force/*=false*/, const float zraise/*=POWER_LOSS_ZRAISE*/, const bool raised/*=false*/) {
  ...
  if (force
    ...
    // SERIAL_ECHOLNPGM("\r\current_position: ", current_position.z, " ", info.current_position.z);
...
void PrintJobRecovery::resume() {
  ...
  const uint32_t resume_sdpos = info.sdpos-4; // Get here before the stepper ISR overwrites it
