...
static void btn_center_event_cb(lv_obj_t *btn, lv_event_t event) {
  ...
  if (DIALOG_IS(TYPE_FILAMENT_LOAD_SELECT)) {
    thermalManager.setTargetHotend(PREHEAT_1_TEMP_HOTEND+10, uiCfg.curSprayerChoose);
    gCfgItems.filament_limit_temper = PREHEAT_1_TEMP_HOTEND+10;
...
void filament_sprayer_temp() {
  ...
  strcpy(public_buf_l, TERN(TFT_MIXWARE_LVGL_UI, MTR.nozzle, uiCfg.curSprayerChoose < 1 ? extrude_menu.ext1 : extrude_menu.ext2));
...
void filament_dialog_handle() {
    ...
    if(temps_update_flag) {
      if (DIALOG_IS(TYPE_FILAMENT_LOAD_HEAT, TYPE_FILAMENT_UNLOAD_HEAT)) {
        ...
        temps_update_flag = false;
        if (uiCfg.leveling_first_time == 1) {
  ...
  if (uiCfg.filament_heat_completed_load == 1) {
    ...
    #else
      sprintf_P(public_buf_m, PSTR("T%d\nG91\nG1 E%d F%d\nG90"), uiCfg.curSprayerChoose, gCfgItems.filamentchange_load_length, (int)(gCfgItems.filamentchange_load_speed*MPRE.extrusion_volume));
    #endif
    queue.inject(public_buf_m);
  }
  else if (uiCfg.filament_heat_completed_unload == 1) {
    ...
    #else
      sprintf_P(public_buf_m, PSTR("T%d\nG91\nG1 E15 F%d\nG1 E-%d F%d\nG90"), uiCfg.curSprayerChoose, (int)(100*MPRE.extrusion_volume), gCfgItems.filamentchange_unload_length, (int)(gCfgItems.filamentchange_unload_speed*MPRE.extrusion_volume));
    #endif
    queue.inject(public_buf_m);
  }

  if (uiCfg.filament_load_heat_flg IF_ENABLED(TFT_MIXWARE_LVGL_UI, && DIALOG_IS(TYPE_FILAMENT_LOAD_HEAT))) {
    const int16_t diff = thermalManager.degHotend(uiCfg.curSprayerChoose) - gCfgItems.filament_limit_temper;
  ...
  else if (uiCfg.filament_unload_heat_flg IF_ENABLED(TFT_MIXWARE_LVGL_UI, && DIALOG_IS(TYPE_FILAMENT_UNLOAD_HEAT))) {
    const int16_t diff = thermalManager.degHotend(uiCfg.curSprayerChoose) - gCfgItems.filament_limit_temper;
    if (ABS(diff) < 2 || diff > 0) {
      uiCfg.filament_unload_heat_flg = 0;
      #if DISABLED(TFT_MIXWARE_LVGL_UI)
