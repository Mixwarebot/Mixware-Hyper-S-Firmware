...
typedef struct move_menu_disp {
  ...
  const char *z_add;
  const char *z_dec;
  TERN_(MIXWARE_MODEL_V, const char *x_axis;)
  TERN_(MIXWARE_MODEL_V, const char *y_axis;)
  TERN_(MIXWARE_MODEL_V, const char *z_axis;)
  const char *step_001mm;
  const char *step_005mm;
  const char *step_01mm;
  const char *step_1mm;
  TERN_(MIXWARE_MODEL_V, const char *step_5mm;)
...
typedef struct home_menu_disp {
  ...
  const char *home_z;
  const char *stopmove;
  TERN_(MIXWARE_MODEL_V, const char *motor_off;)
  const char *back;
} home_menu_def;
...
typedef struct leveling_menu_disp {
  ...
  char *back;
  TERN_(MIXWARE_MODEL_V, const char *z_offset;)
} leveling_menu_def;
...
typedef struct operation_menu_disp {
  ...
  const char *back;
  const char *babystep;
  TERN_(MIXWARE_MODEL_V, const char *filament_sensor_on;)
  TERN_(MIXWARE_MODEL_V, const char *filament_sensor_off;)
} operation_menu_def;
