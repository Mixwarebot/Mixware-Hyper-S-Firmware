...
void lv_draw_acceleration_settings(void) {
  ...
  if (uiCfg.para_ui_page != 1) {
    ...
    lv_screen_menu_item_1_edit(scr, machine_menu.Y_Acceleration, PARA_UI_POS_X, TERN(MIXWARE_MODEL_V, PARA_UI_POS_Y * 5, PARA_UI_POS_Y), event_handler, ID_ACCE_Y, TERN(MIXWARE_MODEL_V, 4, 0), public_buf_l);

    sprintf_P(public_buf_l, PSTR("%d"), (int)planner.settings.max_acceleration_mm_per_s2[Z_AXIS]);
    lv_screen_menu_item_1_edit(scr, machine_menu.Z_Acceleration, PARA_UI_POS_X, TERN(MIXWARE_MODEL_V, PARA_UI_POS_Y * 6, PARA_UI_POS_Y * 2), event_handler, ID_ACCE_Z, TERN(MIXWARE_MODEL_V, 5, 1), public_buf_l);

    sprintf_P(public_buf_l, PSTR("%d"), (int)planner.settings.max_acceleration_mm_per_s2[E_AXIS]);
    lv_screen_menu_item_1_edit(scr, machine_menu.E0_Acceleration, PARA_UI_POS_X, TERN(MIXWARE_MODEL_V, PARA_UI_POS_Y * 7, PARA_UI_POS_Y * 3), event_handler, ID_ACCE_E0, TERN(MIXWARE_MODEL_V, 6, 2), public_buf_l);
