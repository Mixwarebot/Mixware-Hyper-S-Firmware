...
static void event_handler(lv_obj_t *obj, lv_event_t event) {
  ...
  if(!(bool)(queue.length > (BUFSIZE - 3))) {
    bool do_inject = true;
        ...
        if (queue.length <= (BUFSIZE - 3)) {
          switch (MPRE.move_axis) {
            case X_AXIS:
              ...
              sprintf_P(public_buf_l, PSTR("G1 X%s F%d"), dtostrf(cur_pos, 1, 3, str_1), XY_PROBE_SPEED / 2);
              break;
            case Y_AXIS:
              if ((cur_pos = current_position[Y_AXIS] + uiCfg.move_dist) > Y_MAX_POS) cur_pos = Y_MAX_POS;
              sprintf_P(public_buf_l, PSTR("G1 Y%s F%d"), dtostrf(cur_pos, 1, 3, str_1), XY_PROBE_SPEED / 2);
              break;
            case Z_AXIS:
              if ((cur_pos = current_position[Z_AXIS] + uiCfg.move_dist) > Z_MAX_POS) cur_pos = Z_MAX_POS;
              sprintf_P(public_buf_l, PSTR("G1 Z%s F%d"), dtostrf(cur_pos, 1, 3, str_1), Z_PROBE_SPEED_FAST);
              break;
            default: break;
          }
