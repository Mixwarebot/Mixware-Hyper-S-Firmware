  ...
  for (i = 0; i < FILE_BTN_CNT; i++) {
    ...
    */
    ...
    watchdog_refresh();
      ...
      ZERO(public_buf_m);
      cutFileName((char *)list_file.long_name[i], 27, 27, (char *)public_buf_m, list_file.IsFolder[i]);
...
void cutFileName(char *path, int len, int bytePerLine, char *outStr, bool isDir) {
  #if _LFN_UNICODE
    TCHAR *tmpFile;
