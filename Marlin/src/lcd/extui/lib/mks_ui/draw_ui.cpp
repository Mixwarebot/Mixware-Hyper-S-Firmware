...
void gCfgItems_init() {
  ...
  gCfgItems.pausePosX         = TERN(MIXWARE_MODEL_V, 2, -1);
  gCfgItems.pausePosY         = TERN(MIXWARE_MODEL_V, 0, -1);
  gCfgItems.pausePosZ         = TERN(MIXWARE_MODEL_V, 15, 5);
...
void tft_style_init() {
  ...
  style_line.line.color   = TERN(MIXWARE_MODEL_V, LV_COLOR_MAKE(0xF1, 0x5B, 0x20), LV_COLOR_MAKE(0x49, 0x54, 0xFF));
  ...
  style_btn_rel.body.main_color   = TERN(MIXWARE_MODEL_V, TFT_LV_PARA_BACK_BODY_COLOR, lv_color_hex3(0xADF));
  ...
  style_btn_pr.body.border.color = TERN(MIXWARE_MODEL_V, LV_COLOR_BACKGROUND, lv_color_hex3(0x46B));
  style_btn_pr.body.main_color   = TERN(MIXWARE_MODEL_V, TFT_LV_PARA_BACK_BODY_COLOR, lv_color_hex3(0x8BD));
  style_btn_pr.body.grad_color   = TERN(MIXWARE_MODEL_V, TFT_LV_PARA_BACK_BODY_COLOR, lv_color_hex3(0x24A));
  style_btn_pr.body.shadow.width = 2;
  style_btn_pr.text.color        = TERN(MIXWARE_MODEL_V, LV_COLOR_WHITE, lv_color_hex3(0xBCD));
...
char *getDispText(int index) {
  ...
  switch (disp_state_stack._disp_state[index]) {
    ...
    case PRE_HEAT_UI:
      ...
      else strcpy(public_buf_l, TERN(MIXWARE_MODEL_V, operation_menu.temp, preheat_menu.title));
      break;
    case SET_UI:
...
char *creat_title_text() {
  ...
  cutFileName(list_file.long_name[sel_id], TERN(MIXWARE_MODEL_V, 26, 16), TERN(MIXWARE_MODEL_V, 26, 16), tmpCurFileStr);
  ...
  void draw_default_preview(int xpos_pixel, int ypos_pixel, uint8_t sel) {
    ...
    for (index = 0; index < 10; index++) { // 200*200
      ...
      SPI_TFT.setWindow(xpos_pixel, y_off * 20 + ypos_pixel, TERN(MIXWARE_MODEL_V, 160, 200), 20); // 200*200
