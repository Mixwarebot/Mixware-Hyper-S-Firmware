...
void tft_lvgl_init() {
  ...
  lv_disp_buf_init(&disp_buf, bmp_public_buf, nullptr, LV_HOR_RES_MAX * 15); /*Initialize the display buffer*/
  ...
  bool ready = true;
    ...
    if (recovery.valid()) {
      ...
      strncpy(list_file.file_name[sel_id], card.longFilename, sizeof(list_file.file_name[sel_id]));
      lv_draw_printing();
    }
  #endif
...
bool my_touchpad_read(lv_indev_drv_t * indev_driver, lv_indev_data_t * data) {
  ...
  if (diffTime > 20) {
    if (get_point(&last_x, &last_y)) {
      ...
      data->point.x = TERN(MIXWARE_MODEL_V, TFT_WIDTH - last_y, last_x);
      data->point.y = TERN(MIXWARE_MODEL_V, last_x, last_y);;
