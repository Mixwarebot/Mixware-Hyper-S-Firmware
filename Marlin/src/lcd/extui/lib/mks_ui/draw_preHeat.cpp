...
static void event_handler(lv_obj_t *obj, lv_event_t event) {
  ...
  switch (obj->mks_obj_id) {
    ...
    case ID_P_DEC:
      if (uiCfg.curTempType == 0) {
        if (thermalManager.degTargetHotend(uiCfg.curSprayerChoose) > uiCfg.stepHeat)
          thermalManager.temp_hotend[uiCfg.curSprayerChoose].target -= uiCfg.stepHeat;
        else
...
void disp_temp_type() {
  if (uiCfg.curTempType == 0) {
    ...
    if (uiCfg.curSprayerChoose == 1 && (DISABLED(SINGLENOZZLE))) {
...
void disp_desire_temp() {
    ...
    if (uiCfg.curTempType == 0) {
        ...
        sprintf(buf, preheat_menu.value_state, thermalManager.wholeDegHotend(uiCfg.curSprayerChoose), thermalManager.degTargetHotend(uiCfg.curSprayerChoose));
      #endif
    }
    else {
      #if HAS_HEATED_BED
