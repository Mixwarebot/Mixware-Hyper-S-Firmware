    ...
    case ID_FILAMNT_OUT:
      uiCfg.filament_unload_heat_flg = true;
        ...
        if (thermalManager.degTargetHotend(uiCfg.extruderIndex)
            && (ABS(thermalManager.degTargetHotend(uiCfg.extruderIndex) - thermalManager.degHotend(uiCfg.extruderIndex)) <= 1
...
void disp_filament_temp() {
  ...
  sprintf(buf, preheat_menu.value_state, thermalManager.degHotend(uiCfg.curSprayerChoose), thermalManager.degTargetHotend(uiCfg.curSprayerChoose));
