...
static void disp_key_value() {
  ...
  switch (value) {
    ...
    case filament_temp_set:
      sprintf_P(public_buf_m, PSTR("%d"), thermalManager.temp_hotend[0].target);
      break;
    case extrude_min_temp:
      itoa(thermalManager.extrude_min_temp, public_buf_m, 10);
      break;
    #if ENABLED(INPUT_SHAPING_X)
      case is_freq_x: dtostrf(stepper.get_shaping_frequency(X_AXIS), 1, 2, public_buf_m); break;
    #endif
    #if ENABLED(INPUT_SHAPING_Y)
      case is_freq_y: dtostrf(stepper.get_shaping_frequency(Y_AXIS), 1, 2, public_buf_m); break;
    #endif
  }
...
static void set_value_confirm() {
  if (value == filament_temp_set ) {
    ...
    LIMIT(temper, 0, gCfgItems.filament_max_temper);
  ...
  switch (value) {
    ...
    case unload_length:
      ...
      uiCfg.filament_unloading_time = (uint32_t)((UNLOAD_PRELOAD_TIME+gCfgItems.filamentchange_unload_length*60.0/gCfgItems.filamentchange_unload_speed)+0.5);
      update_spi_flash();
      break;
    case unload_speed:
    ...
    case extrude_min_temp:
      thermalManager.extrude_min_temp = atoi(key_value);
      NOLESS(thermalManager.extrude_min_temp, 0);
      thermalManager.allow_cold_extrude = (thermalManager.extrude_min_temp == 0);
      gCfgItems.extru_min_temp = thermalManager.extrude_min_temp;
      break;
    #if ENABLED(INPUT_SHAPING_X)
      case is_freq_x:
        {
          uint16_t freqx = atoi(key_value);
          LIMIT(freqx, SHAPING_MIN_FREQ, SHAPING_MAX_FREQ);
          stepper.set_shaping_frequency(X_AXIS, freqx);
        }
        break;
    #endif
    #if ENABLED(INPUT_SHAPING_Y)
      case is_freq_y:
        {
          uint16_t freqy = atoi(key_value);
          LIMIT(freqy, SHAPING_MIN_FREQ, SHAPING_MAX_FREQ);
          stepper.set_shaping_frequency(Y_AXIS, freqy);
        }
        break;
    #endif
  }
...
void lv_draw_number_key() {
  ...
  #define NUMBER_KEY_INTERVAL       3
  #define NUMBER_KEY_SIZE_WIDTH     76
  #define NUMBER_KEY_SIZE_HEIGHT    87
  #define NUMBER_KEY_POS_X_1        4
  #define NUMBER_KEY_POS_X_2        NUMBER_KEY_INTERVAL + NUMBER_KEY_SIZE_WIDTH + NUMBER_KEY_POS_X_1
  #define NUMBER_KEY_POS_X_3        NUMBER_KEY_INTERVAL + NUMBER_KEY_SIZE_WIDTH + NUMBER_KEY_POS_X_2
  #define NUMBER_KEY_POS_X_4        NUMBER_KEY_INTERVAL + NUMBER_KEY_SIZE_WIDTH + NUMBER_KEY_POS_X_3
  #define NUMBER_KEY_POS_Y_1        120
  #define NUMBER_KEY_POS_Y_2        NUMBER_KEY_INTERVAL + NUMBER_KEY_SIZE_HEIGHT + NUMBER_KEY_POS_Y_1
  #define NUMBER_KEY_POS_Y_3        NUMBER_KEY_INTERVAL + NUMBER_KEY_SIZE_HEIGHT + NUMBER_KEY_POS_Y_2
  #define NUMBER_KEY_POS_Y_4        NUMBER_KEY_INTERVAL + NUMBER_KEY_SIZE_HEIGHT + NUMBER_KEY_POS_Y_3
  #define LV_DRAW_NUMBERKEY_V(B, L, ID, X, Y, W, H, T)  do{\
                                                          B = lv_btn_create(scr, X, Y, W, H, event_handler, ID, &style_num_key_pre);\
                                                          L = lv_label_create_empty(B);\
                                                          lv_label_set_text(L, T);\
                                                          lv_obj_align(L, B, LV_ALIGN_CENTER, 0, 0);\
                                                        }while (0)

  lv_obj_t *NumberKey_1, *labelKey_1;
  lv_obj_t *NumberKey_2, *labelKey_2;
  lv_obj_t *NumberKey_3, *labelKey_3;
  lv_obj_t *NumberKey_4, *labelKey_4;
  lv_obj_t *NumberKey_5, *labelKey_5;
  lv_obj_t *NumberKey_6, *labelKey_6;
  lv_obj_t *NumberKey_7, *labelKey_7;
  lv_obj_t *NumberKey_8, *labelKey_8;
  lv_obj_t *NumberKey_9, *labelKey_9;
  lv_obj_t *NumberKey_0, *labelKey_0;
  lv_obj_t *KeyBack,     *labelKeyBack;
  lv_obj_t *KeyReset,    *labelKeyReset;
  lv_obj_t *KeyConfirm,  *labelKeyConfirm;
  lv_obj_t *KeyPoint,    *labelKeyPoint;
  lv_obj_t *Minus,       *labelMinus;

  buttonValue = lv_btn_create(scr, 4, 3, 313, 114, event_handler, 0, &style_num_text);
  labelValue = lv_label_create_empty(buttonValue);
  LV_DRAW_NUMBERKEY_V(NumberKey_1, labelKey_1,      ID_NUM_KEY1,     NUMBER_KEY_POS_X_1, NUMBER_KEY_POS_Y_3, NUMBER_KEY_SIZE_WIDTH, NUMBER_KEY_SIZE_HEIGHT, machine_menu.key_1);
  LV_DRAW_NUMBERKEY_V(NumberKey_2, labelKey_2,      ID_NUM_KEY2,     NUMBER_KEY_POS_X_2, NUMBER_KEY_POS_Y_3, NUMBER_KEY_SIZE_WIDTH, NUMBER_KEY_SIZE_HEIGHT, machine_menu.key_2);
  LV_DRAW_NUMBERKEY_V(NumberKey_3, labelKey_3,      ID_NUM_KEY3,     NUMBER_KEY_POS_X_3, NUMBER_KEY_POS_Y_3, NUMBER_KEY_SIZE_WIDTH, NUMBER_KEY_SIZE_HEIGHT, machine_menu.key_3);
  LV_DRAW_NUMBERKEY_V(NumberKey_4, labelKey_4,      ID_NUM_KEY4,     NUMBER_KEY_POS_X_1, NUMBER_KEY_POS_Y_2, NUMBER_KEY_SIZE_WIDTH, NUMBER_KEY_SIZE_HEIGHT, machine_menu.key_4);
  LV_DRAW_NUMBERKEY_V(NumberKey_5, labelKey_5,      ID_NUM_KEY5,     NUMBER_KEY_POS_X_2, NUMBER_KEY_POS_Y_2, NUMBER_KEY_SIZE_WIDTH, NUMBER_KEY_SIZE_HEIGHT, machine_menu.key_5);
  LV_DRAW_NUMBERKEY_V(NumberKey_6, labelKey_6,      ID_NUM_KEY6,     NUMBER_KEY_POS_X_3, NUMBER_KEY_POS_Y_2, NUMBER_KEY_SIZE_WIDTH, NUMBER_KEY_SIZE_HEIGHT, machine_menu.key_6);
  LV_DRAW_NUMBERKEY_V(NumberKey_7, labelKey_7,      ID_NUM_KEY7,     NUMBER_KEY_POS_X_1, NUMBER_KEY_POS_Y_1, NUMBER_KEY_SIZE_WIDTH, NUMBER_KEY_SIZE_HEIGHT, machine_menu.key_7);
  LV_DRAW_NUMBERKEY_V(NumberKey_8, labelKey_8,      ID_NUM_KEY8,     NUMBER_KEY_POS_X_2, NUMBER_KEY_POS_Y_1, NUMBER_KEY_SIZE_WIDTH, NUMBER_KEY_SIZE_HEIGHT, machine_menu.key_8);
  LV_DRAW_NUMBERKEY_V(NumberKey_9, labelKey_9,      ID_NUM_KEY9,     NUMBER_KEY_POS_X_3, NUMBER_KEY_POS_Y_1, NUMBER_KEY_SIZE_WIDTH, NUMBER_KEY_SIZE_HEIGHT, machine_menu.key_9);
  LV_DRAW_NUMBERKEY_V(NumberKey_0, labelKey_0,      ID_NUM_KEY0,     NUMBER_KEY_POS_X_2, NUMBER_KEY_POS_Y_4, NUMBER_KEY_SIZE_WIDTH, NUMBER_KEY_SIZE_HEIGHT, machine_menu.key_0);
  LV_DRAW_NUMBERKEY_V(KeyBack,     labelKeyBack,    ID_NUM_BACK,     NUMBER_KEY_POS_X_4, NUMBER_KEY_POS_Y_1, NUMBER_KEY_SIZE_WIDTH, NUMBER_KEY_SIZE_HEIGHT, machine_menu.key_back);
  LV_DRAW_NUMBERKEY_V(KeyReset,    labelKeyReset,   ID_NUM_RESET,    NUMBER_KEY_POS_X_4, NUMBER_KEY_POS_Y_2, NUMBER_KEY_SIZE_WIDTH, NUMBER_KEY_SIZE_HEIGHT, machine_menu.key_reset);
  LV_DRAW_NUMBERKEY_V(KeyConfirm,  labelKeyConfirm, ID_NUM_CONFIRM,  NUMBER_KEY_POS_X_4, NUMBER_KEY_POS_Y_3, NUMBER_KEY_SIZE_WIDTH, NUMBER_KEY_SIZE_HEIGHT * 2 + NUMBER_KEY_INTERVAL, machine_menu.key_confirm);
  LV_DRAW_NUMBERKEY_V(KeyPoint,    labelKeyPoint,   ID_NUM_POINT,    NUMBER_KEY_POS_X_3, NUMBER_KEY_POS_Y_4, NUMBER_KEY_SIZE_WIDTH, NUMBER_KEY_SIZE_HEIGHT, machine_menu.key_point);
  LV_DRAW_NUMBERKEY_V(Minus,       labelMinus,      ID_NUM_NEGATIVE, NUMBER_KEY_POS_X_1, NUMBER_KEY_POS_Y_4, NUMBER_KEY_SIZE_WIDTH, NUMBER_KEY_SIZE_HEIGHT, machine_menu.negative);
