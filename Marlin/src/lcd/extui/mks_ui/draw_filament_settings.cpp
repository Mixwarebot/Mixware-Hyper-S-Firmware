...
void lv_draw_filament_settings() {
    ...
    itoa(gCfgItems.filamentchange_load_length, public_buf_l, 10);
    M_SCREEN_EDITITEM(MTR.ADVSetFilInLength,  ID_FILAMENT_SET_IN_LENGTH, index);
    index++;

    itoa(gCfgItems.filamentchange_load_speed, public_buf_l, 10);
    M_SCREEN_EDITITEM(MTR.ADVSetFilInSpeed,   ID_FILAMENT_SET_IN_SPEED, index);
    index++;

    itoa(gCfgItems.filamentchange_unload_length, public_buf_l, 10);
    M_SCREEN_EDITITEM(MTR.ADVSetFilOutLength, ID_FILAMENT_SET_OUT_LENGTH, index);
    index++;
