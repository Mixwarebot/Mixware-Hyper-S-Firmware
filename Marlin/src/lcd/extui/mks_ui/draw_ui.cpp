...
lv_point_t line_points[LINE_MAX_NUMBERS][2] = {
  {{PARA_UI_POS_X, PARA_UI_POS_Y*1 + titleHeight}, {TFT_WIDTH-PARA_UI_POS_X, PARA_UI_POS_Y*1 + titleHeight}},
  {{PARA_UI_POS_X, PARA_UI_POS_Y*2 + titleHeight}, {TFT_WIDTH-PARA_UI_POS_X, PARA_UI_POS_Y*2 + titleHeight}},
  {{PARA_UI_POS_X, PARA_UI_POS_Y*3 + titleHeight}, {TFT_WIDTH-PARA_UI_POS_X, PARA_UI_POS_Y*3 + titleHeight}},
  {{PARA_UI_POS_X, PARA_UI_POS_Y*4 + titleHeight}, {TFT_WIDTH-PARA_UI_POS_X, PARA_UI_POS_Y*4 + titleHeight}},
  {{PARA_UI_POS_X, PARA_UI_POS_Y*5 + titleHeight}, {TFT_WIDTH-PARA_UI_POS_X, PARA_UI_POS_Y*5 + titleHeight}},
  {{PARA_UI_POS_X, PARA_UI_POS_Y*6 + titleHeight}, {TFT_WIDTH-PARA_UI_POS_X, PARA_UI_POS_Y*6 + titleHeight}}
};
...
void gCfgItems_init() {
  ...
  gCfgItems.extru_min_temp = EXTRUDE_MINTEMP;
  ...
  thermalManager.extrude_min_temp = gCfgItems.extru_min_temp;
}
...
void ui_cfg_init() {
  ...
  uiCfg.filament_unloading_time = (uint32_t)((UNLOAD_PRELOAD_TIME+gCfgItems.filamentchange_unload_length * 60.0f / gCfgItems.filamentchange_unload_speed) + 0.5f);
...
void tft_style_init() {
  ...
  style_line.line.color   = LV_COLOR_MAKE(0xF1, 0x5B, 0x20);
  ...
  style_btn_rel.body.border.color = LV_COLOR_BACKGROUND;
  style_btn_rel.body.border.width = 1;
  style_btn_rel.body.main_color   = TFT_LV_PARA_BACK_BODY_COLOR;
  style_btn_rel.body.grad_color   = TFT_LV_PARA_BACK_BODY_COLOR;
  ...
  style_btn_pr.body.border.color = LV_COLOR_BACKGROUND;
  style_btn_pr.body.main_color   = TFT_LV_PARA_BACK_BODY_COLOR;
  style_btn_pr.body.grad_color   = TFT_LV_PARA_BACK_BODY_COLOR;
  ...
  style_num_key_pre.body.radius = 3;
  style_num_text.body.radius = 3;
  ...
  switch (disp_state_stack._disp_state[index]) {
    ...
    case PREHEAT_UI:
      switch (disp_state_stack._disp_state[disp_state_stack._disp_index]) {
        ...
        default:              strcpy(public_buf_l, operation_menu.temp); break;
      }
      break;
...
char *creat_title_text() {
  ...
  void draw_default_preview(int xpos_pixel, int ypos_pixel, uint8_t sel) {
    ...
    for (index = 0; index < 10; index++) { // 200*200
      ...
      SPI_TFT.setWindow(xpos_pixel, y_off * 20 + ypos_pixel, 160, 20); // 200*200
...
void GUI_RefreshPage() {
  ...
  if ((systick_uptime_millis % 1500) == 0) printing_rate_update_flag = true;
  #if ENABLED(TFT_MIXWARE_UI)
...
void clear_cur_ui() {
  ...
  switch (disp_state_stack._disp_state[disp_state_stack._disp_index]) {
    ...
    case EEPROM_SETTINGS_UI:          lv_clear_eeprom_settings(); break;
      ...
      case IS_FREQ_UI:                lv_clear_is_freq_settings(); break;
    default: break;
  }
}
  ...
  if (disp_state_stack._disp_index > 0) {
    ...
    switch (disp_state_stack._disp_state[disp_state_stack._disp_index]) {
      ...
      case EEPROM_SETTINGS_UI:          lv_draw_eeprom_settings(); break;
        ...
        case IS_FREQ_UI:                lv_draw_is_freq_settings(); break;
      default: break;
    }
  }
}
...
lv_obj_t* lv_screen_menu_item(lv_obj_t *par, const char *text, lv_coord_t x, lv_coord_t y, lv_event_cb_t cb, const int id, const int index, bool drawArrow) {
  ...
  if (drawArrow) (void)lv_imgbtn_create(par, "F:/bmp_arrow.bin", x + PARA_UI_SIZE_X - BTN_ARROW_WIDTH - PARA_UI_POS_X, y + PARA_UI_SIZE_Y/2 - BTN_ARROW_HEIGHT/2, cb, id);

  if (index < LINE_MAX_NUMBERS) {
...
void LV_TASK_HANDLER() {
  ...
  TERN_(HAS_GCODE_PREVIEW, disp_pre_gcode(0, 80));
...
#if ENABLED(TFT_MIXWARE_UI)
bool enabled_thermal_protection;
...
lv_point_t button_pixel_point[6] = {
  ...
  {BUTTON_POS_X, BUTTON_POS_Y + BTN_Y_PIXEL*1}, {BUTTON_POS_X*2 + BTN_X_PIXEL + INTERVAL_V, BUTTON_POS_Y + BTN_Y_PIXEL*1},
  {BUTTON_POS_X, BUTTON_POS_Y + BTN_Y_PIXEL*2}, {BUTTON_POS_X*2 + BTN_X_PIXEL + INTERVAL_V, BUTTON_POS_Y + BTN_Y_PIXEL*2}
};
...
void lv_screen_menu_item_turn_page(lv_obj_t *par, const char *text, lv_event_cb_t cb, const int id) {
  lv_obj_t* btnTurnPage = lv_btn_create(par, PARA_UI_TURN_PAGE_POS_X, PARA_UI_TURN_PAGE_POS_Y, PARA_UI_BACK_BTN_X_SIZE, PARA_UI_BACK_BTN_Y_SIZE, cb, id);
