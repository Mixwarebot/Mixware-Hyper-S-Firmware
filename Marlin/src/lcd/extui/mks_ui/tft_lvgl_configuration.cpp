...
void SysTick_Callback() {
  ...
  if (uiCfg.filament_loading_time_flg) {
    ...
    uiCfg.filament_rate = uint32_t(100.0f * uiCfg.filament_loading_time_cnt / SEC_TO_MS(uiCfg.filament_loading_time / uiCfg.filament_flow) + 0.5f);
    if (uiCfg.filament_loading_time_cnt >= SEC_TO_MS(uiCfg.filament_loading_time / uiCfg.filament_flow)) {
  ...
  if (uiCfg.filament_unloading_time_flg) {
    ...
    uiCfg.filament_rate = uint32_t(100.0f * uiCfg.filament_unloading_time_cnt / SEC_TO_MS(uiCfg.filament_unloading_time / uiCfg.filament_flow) + 0.5f);
    if (uiCfg.filament_unloading_time_cnt >= SEC_TO_MS(uiCfg.filament_unloading_time / uiCfg.filament_flow)) {
...
void tft_lvgl_init() {
  ...
  lv_disp_buf_init(&disp_buf, bmp_public_buf, nullptr, MAX(LV_HOR_RES_MAX, LV_VER_RES_MAX) * 14); // Initialize the display buffer
  ...
  if (READ(Z_MIN_PROBE_PIN) == false) has_probe = true;
}
...
bool my_touchpad_read(lv_indev_drv_t * indev_driver, lv_indev_data_t * data) {
  ...
  if (get_point(&last_x, &last_y)) {
    #if TFT_ROTATION == TFT_ROTATE_90
      data->point.x = TFT_WIDTH - last_y;
      data->point.y = last_x;
    #elif TFT_ROTATION == TFT_ROTATE_180
  ...
  #endif
    // SERIAL_ECHOLNPGM("\r\nTOUCH_CALIBRATION_X ", data->point.x);
    // SERIAL_ECHOLNPGM("TOUCH_CALIBRATION_Y ", data->point.y);
