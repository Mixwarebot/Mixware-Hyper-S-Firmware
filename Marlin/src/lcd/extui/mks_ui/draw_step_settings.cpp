...
void lv_draw_step_settings() {
  ...
  // if (!uiCfg.para_ui_page) {
    ...
    lv_screen_menu_item_1_edit(scr, machine_menu.X_Steps, PARA_UI_POS_X, PARA_UI_MENU_ITEM_Y(0), event_handler, ID_STEP_X, 0, public_buf_l);

    dtostrf(planner.settings.axis_steps_per_mm[Y_AXIS], 1, 1, public_buf_l);
    lv_screen_menu_item_1_edit(scr, machine_menu.Y_Steps, PARA_UI_POS_X, PARA_UI_MENU_ITEM_Y(1), event_handler, ID_STEP_Y, 1, public_buf_l);

    dtostrf(planner.settings.axis_steps_per_mm[Z_AXIS], 1, 1, public_buf_l);
    lv_screen_menu_item_1_edit(scr, machine_menu.Z_Steps, PARA_UI_POS_X, PARA_UI_MENU_ITEM_Y(2), event_handler, ID_STEP_Z, 2, public_buf_l);

    dtostrf(planner.settings.axis_steps_per_mm[E_AXIS], 1, 1, public_buf_l);
    lv_screen_menu_item_1_edit(scr, machine_menu.E0_Steps, PARA_UI_POS_X, PARA_UI_MENU_ITEM_Y(3), event_handler, ID_STEP_E0, 3, public_buf_l);
