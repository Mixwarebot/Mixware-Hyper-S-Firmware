...
void lv_draw_operation() {
    ...
    if (uiCfg.print_state != WORKING) {
      lv_big_button_create(scr, MIMG.move, MTR.move, IMAGEBTN_P_X(4), IMAGEBTN_P_Y(4), event_handler, ID_O_MOV);
    }
    else {
      lv_big_button_create(scr, MIMG.speed, MTR.speed,    IMAGEBTN_P_X(4), IMAGEBTN_P_Y(4), event_handler, ID_O_SPEED);
