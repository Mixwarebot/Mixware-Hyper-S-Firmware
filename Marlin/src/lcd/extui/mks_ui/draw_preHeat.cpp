  ...
  switch (obj->mks_obj_id) {
    ...
    case ID_P_ADD:
      if (uiCfg.curTempType == 0) {
        thermalManager.setTargetHotend(thermalManager.temp_hotend[uiCfg.extruderIndex].target + uiCfg.stepHeat, uiCfg.extruderIndex);
        thermalManager.start_watching_hotend(uiCfg.extruderIndex);
      }
      else {
          thermalManager.setTargetBed(thermalManager.temp_bed.target + uiCfg.stepHeat);
...
void lv_draw_preHeat() {
  scr = lv_screen_create(PREHEAT_UI);
    ...
    lv_obj_set_pos(buttonNull,  0, TFT_HEIGHT - 30);
    ...
    if (!uiCfg.para_ui_page) {
      ...
      lv_big_button_create(scr, "F:/img_preheat_Cool.bin", preheat_menu.off,     button_pixel_point[5].x, button_pixel_point[5].y, event_handler, ID_P_OFF);
    ...
    lv_img_set_src(buttonExt, get_heating_mode() ? "F:/bmp_ext_state_2.bin" : "F:/HI_ext_state_2.bin");
...
void disp_temp_type() {
  if (uiCfg.curTempType == 0) {
    ...
    if (ENABLED(HAS_HOTEND)) {
      lv_imgbtn_set_src_both(buttonType, get_heating_mode() ? "F:/img_extruct.bin" : "F:/HI_extruct.bin");
      if (gCfgItems.multiple_language) {
    ...
    lv_imgbtn_set_src_both(buttonType, "F:/img_bed.bin");
    if (gCfgItems.multiple_language) {
...
void disp_desire_temp() {
  #if ENABLED(TFT_MIXWARE_UI)
    sprintf(public_buf_l, preheat_menu.value_state, thermalManager.wholeDegHotend(uiCfg.extruderIndex), thermalManager.degTargetHotend(uiCfg.extruderIndex));
    lv_label_set_text(labelExt, public_buf_l);
    sprintf(public_buf_l, preheat_menu.value_state, thermalManager.wholeDegBed(), thermalManager.degTargetBed());
...
void disp_step_heat() {
  if (uiCfg.stepHeat == 1) {
    lv_imgbtn_set_src_both(buttonStep, "F:/img_step_degree_1.bin");
  }
  else if (uiCfg.stepHeat == 5) {
    lv_imgbtn_set_src_both(buttonStep, "F:/img_step_degree_5.bin");
  }
  else if (uiCfg.stepHeat == 10) {
    lv_imgbtn_set_src_both(buttonStep, "F:/img_step_degree_10.bin");
  }
