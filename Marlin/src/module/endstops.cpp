...
void Endstops::update() {
   ...
   */
    ...
    #if ENABLED(MIXWARE_MODEL_V)
      if ((READ(_ENDSTOP_PIN(Z2, MIN)) != _ENDSTOP_INVERTING(Z2, MIN) && (level_state == LEVEL_STATE_LEVELING)) || (READ(_ENDSTOP_PIN(Z, MIN)) != _ENDSTOP_INVERTING(Z, MIN) && (level_state != LEVEL_STATE_LEVELING)))
        SBI(live_state, _ENDSTOP(Z, MIN));
      else
        CBI(live_state, _ENDSTOP(Z, MIN));
    #else
  ...
  #if ENABLED(MIXWARE_MODEL_V)
    #define PROCESS_ENDSTOP_MMV_Z(AXIS, MINMAX) do { \
      if (TEST_ENDSTOP(_ENDSTOP(AXIS, MINMAX)) || TEST_ENDSTOP(_ENDSTOP(AXIS##2, MINMAX))) { \
        _ENDSTOP_HIT(AXIS, MINMAX); \
        planner.endstop_triggered(_AXIS(AXIS)); \
      } \
    }while(0)
  ...
  #if ENABLED(MIXWARE_MODEL_V)
    #define PROCESS_ENDSTOP_Z(MINMAX) PROCESS_ENDSTOP_MMV_Z(Z, MINMAX)
  #else
