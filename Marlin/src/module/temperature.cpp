...
void Temperature::manage_heater() {
  ...
  millis_t ms = millis();
    ...
    if (degHotend(0) > 60) analogWrite(pin_t(NOT_FAN_PIN), 255);//NF
    else analogWrite(pin_t(NOT_FAN_PIN), 0);
    HOTEND_LOOP() {
    ...
    do {
      ...
      #endif
      {
          ...
          if (WITHIN(temp_bed.celsius, BED_MINTEMP, BED_MAXTEMP)) {
              ...
              // Mixware Bed 280W
              // temp_bed.soft_pwm_amount = temp_bed.celsius < temp_bed.target ?
              // (temp_bed.celsius < 70 ? 208
              //   : temp_bed.celsius < 80 ? 222
              //   : temp_bed.celsius < 90 ? 237 : 250) >> 1 : 0;
...
void Temperature::init() {
  ...
  TERN_(MIXWARE_MODEL_V, INIT_FAN_PIN(NOT_FAN_PIN));
  TERN_(MIXWARE_MODEL_V, INIT_FAN_PIN(BOARD_FAN_PIN));
  TERN_(MIXWARE_MODEL_V, INIT_FAN_PIN(MLED1_PIN));
  TERN_(MIXWARE_MODEL_V, INIT_FAN_PIN(MLED2_PIN));
  TERN_(MIXWARE_MODEL_V, INIT_FAN_PIN(MLED3_PIN));

  analogWrite(pin_t(NOT_FAN_PIN), 0);
  analogWrite(pin_t(BOARD_FAN_PIN), 0);
  analogWrite(pin_t(MLED1_PIN), 0);
  analogWrite(pin_t(MLED2_PIN), 0);
  analogWrite(pin_t(MLED3_PIN), 0);
  #if EARLY_WATCHDOG
