...
void Temperature::mintemp_error(const heater_id_t heater_id) {
  ...
  void Temperature::manage_hotends(const millis_t &ms) {
    HOTEND_LOOP() {
        ...
        TERN_(TFT_MIXWARE_UI, if (get_thermal_protection()))
          ...
          temp_bed.soft_pwm_amount = MAX_BED_POWER >> 1;
            ...
            if (WITHIN(temp_bed.celsius, BED_MINTEMP, BED_MAXTEMP)) {
                ...
                if (temp_bed.is_above_target(BED_HYSTERESIS))
                  ...
                  temp_bed.soft_pwm_amount = MAX_BED_POWER >> 1;
              #elif ENABLED(TFT_MIXWARE_UI)
                // Mixware Bed 350W
                temp_bed.soft_pwm_amount = temp_bed.celsius < temp_bed.target ? (  temp_bed.celsius < 60 ? 165
                                                                                : temp_bed.celsius < 70 ? 175
                                                                                : temp_bed.celsius < 80 ? 185
                                                                                : temp_bed.celsius < 90 ? 193
                                                                                                        : 198 ) >> 1 : 0;
